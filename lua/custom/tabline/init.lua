...
local function setup_autocmds()
  ...
  nvim_utils.autocmd("VimEnter", {
    ...
    callback = function()
      if not _config.persist.restore_on_startup then
        return
      end
      ...
      vim.schedule(function()
        ...
        for _, b in ipairs(vim.api.nvim_list_bufs()) do
          if vim.fn.buflisted(b) == 1 and vim.api.nvim_buf_get_name(b) ~= "" then
            return
          end
        end
...
function M.session_list_print()
  ...
  for _, s in ipairs(sessions) do
    ...
    lines[#lines + 1] = string.format("  %s  [%d bufs]  %s", ts, s.count or 0, s.cwd)
  end
