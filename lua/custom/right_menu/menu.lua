...
function Menu:new(get_items, format, actions, opts)
  ...
  local menu = {
    ...
    prev_cursor = vim.api.nvim_win_get_cursor(0),
  }
...
function Menu:set_keymaps()
  for key, action in pairs(self.actions) do
    vim.keymap.set('n', key, function()
      ...
      -- Switch to previous context to get items and perform action
      local ok_win = vim.api.nvim_win_is_valid(self.prev_win)
      if ok_win then
        vim.api.nvim_set_current_win(self.prev_win)
      end

      local items = self.get_items()
      local current_item = items[line]

      if not current_item then
        if ok_win and vim.api.nvim_win_is_valid(self.win) then
