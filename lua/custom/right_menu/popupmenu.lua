...
local function perform_action(item)
  ...
  elseif item == "Git blame" then
    ...
    elseif pcall(vim.cmd, "Git blame") then
      -- Fugitive available
    else
      vim.cmd("!git blame " .. vim.fn.shellescape(vim.fn.expand("%")))
    end
  elseif item == "Git status" then
    local lg_ok, lg = pcall(require, "custom.lazygit")
    if lg_ok then
      local term_ok, term = pcall(require, "custom.float_term.term")
      if term_ok then
        term.create_terminal("lazygit")
      else
        vim.cmd("Git")
      end
    elseif pcall(vim.cmd, "Git") then
      -- Fugitive
    else
      vim.cmd("!git status")
    end
  elseif item == "Git diff" then
    ...
    if gs_ok then
      gs.diffthis()
    elseif pcall(vim.cmd, "Gdiffsplit") then
      -- Fugitive
    else
      vim.cmd("!git diff")
    end
  ...
  elseif item == "Back" then
    -- Use Ctrl-O to go back in jump list
