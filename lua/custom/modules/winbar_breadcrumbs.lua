...
-- Get LSP kind icon using nvim-web-devicons
local function get_lsp_kind_icon(kind)
	...
	-- Try to get an icon for the kind; fall back to a default
	local icon, hl = webdevicons.get_icon(kind_name)
	if not icon then
		-- fallback icons if WebDevIcons doesn't have one for the kind
		...
		icon = fallback_icons[kind_name] or ""
		hl = "Identifier"
	end
...
-- Build breadcrumb parts with icons and highlights using nvim-web-devicons
local function build_breadcrumb_parts(filepath, context_symbols, win)
	...
	local ok, webdevicons = pcall(require, "nvim-web-devicons")
	local icons_enabled = config.icons.enabled

	local bufnr = vim.api.nvim_win_get_buf(win)
	local full_filename = vim.fn.fnamemodify(filepath, ":t")
	if full_filename == "" then
		return parts
	end

	local filename = vim.fn.fnamemodify(full_filename, ":r")
	local ext = full_filename:match("^.+%.(.+)$")

	local icon, hl_group = nil, nil
	if icons_enabled and ok then
		icon, hl_group = webdevicons.get_icon(full_filename, ext, { default = true })
		if not icon then
			icon, hl_group = "󰈔", "Normal"
		end
	elseif icons_enabled then
		icon, hl_group = "󰈔", "Normal"
	end

	table.insert(parts, {
		text = filename,
		highlight = "BreadcrumbsBasename",
		icon = icon,
	...
	if context_symbols and #context_symbols > 1 then
		for i = 2, #context_symbols do
			local symbol = context_symbols[i]
			if symbol.name and symbol.name ~= "" and not symbol.name:match("^%s*$") then
				local kind_icon, kind_hl = nil, nil
				if config.icons.kinds and ok then
					kind_icon, kind_hl = get_lsp_kind_icon(symbol.kind)
				end

				table.insert(parts, {
					text = symbol.name,
					highlight = "BreadcrumbsSymbol", -- New: dedicated hl for symbols
					icon = kind_icon,
...
--- Convert breadcrumb parts to display string
local function parts_to_display_string(parts)
	...
	table.insert(result_parts, " ") -- Leading space

	for i, part in ipairs(parts) do
		-- Icon (if enabled)
		if part.icon and part.icon_highlight and config.icons.enabled then
			table.insert(result_parts, "%#" .. part.icon_highlight .. "#" .. part.icon .. "%*")
			table.insert(result_parts, " ")
		end

		-- Text with part-specific hl
		local text_hl = part.highlight or "WinBar" -- Fallback to WinBar
		table.insert(result_parts, "%#" .. text_hl .. "#" .. part.text .. "%*")

		-- Separator (dim hl, no mode tint)
		if i < #parts then
			local sep_hl = "BreadcrumbsSeparator"
			table.insert(result_parts, "%#" .. sep_hl .. "#" .. separator .. "%*")
		end
	end

	table.insert(result_parts, " ") -- Trailing space
