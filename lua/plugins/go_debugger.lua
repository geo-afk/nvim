...
function M.setup(opts)
  ...
  local function register_keymaps()
    ...
    map("n", "d", function()
      ...
      vim.cmd("GoDlvDebug")
    end, "debug main package")
    map("n", "t", function()
      install_session_keys()
      vim.cmd("GoDlvTest")
    end, "debug test package")
    map("n", "b", function()
      install_session_keys()
      vim.cmd("GoDlvAttachSpawn")
    end, "build + debug binary")
    map("n", "a", function()
      install_session_keys()
      vim.ui.input({ prompt = "PID: " }, function(v)
        if v and v ~= "" then
          vim.cmd("GoDlvAttach " .. v)
        end
      end)
    end, "attach to pid")
    map("n", "C", function()
      ...
      vim.ui.input({ prompt = "Connect (host:port): " }, function(v)
        if v and v ~= "" then
          vim.cmd("GoDlvConnect " .. v)
        end
      end)
    end, "connect to headless dlv")
    map("n", "r", function()
      install_session_keys()
      vim.cmd("GoDlvRestart")
    end, "restart session")
    map("n", "q", function()
      remove_session_keys()
      vim.cmd("GoDlvStop")
