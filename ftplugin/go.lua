-- =============================================================================
--  go.lua — Neovim Go development utilities
...
local TOOLS = {
  ...
  { cmd = 'fillstruct', required = false, install = 'go install github.com/davidrjenni/reftools/cmd/fillstruct@latest' },
  { cmd = 'fillswitch', required = false, install = 'go install github.com/davidrjenni/reftools/cmd/fillswitch@latest' },
...
local function check_tools()
  ...
  vim.schedule(function()
    for _, tool in ipairs(missing_required) do
      vim.notify(string.format('[go.lua] ✗ Required tool missing: %s\n  Install: %s', tool.cmd, tool.install), vim.log.levels.ERROR)
    end
...
local function run_fillstruct()
  ...
  if exit ~= 0 then
    vim.notify('[go.lua] fillstruct CLI failed (may be Go ≥1.25 incompatibility) — trying gopls…', vim.log.levels.WARN)
    fillstruct_via_lsp()
    return
  end
...
local function run_govulncheck(args)
  if vim.fn.executable 'govulncheck' == 0 then
    vim.notify('[go.lua] govulncheck not found.\n  Install: go install golang.org/x/vuln/cmd/govulncheck@latest', vim.log.levels.ERROR)
    return
  end
...
local function setup_dap()
  ...
  if vim.fn.executable 'dlv' == 0 then
    vim.notify('[go.lua] dlv not found.\n  Install: go install github.com/go-delve/delve/cmd/dlv@latest', vim.log.levels.ERROR)
    return false
  end
...
local function run_tests_for_file(filepath)
  ...
  if main_file and test_file then
    vim.notify(string.format('[go.lua] Running tests: %s + %s', vim.fn.fnamemodify(main_file, ':t'), vim.fn.fnamemodify(test_file, ':t')), vim.log.levels.INFO)
  elseif test_file then
    vim.notify(string.format('[go.lua] Running test file: %s (no main file found)', vim.fn.fnamemodify(test_file, ':t')), vim.log.levels.WARN)
  else
    vim.notify(string.format('[go.lua] Running file: %s (no test file found)', vim.fn.fnamemodify(main_file, ':t')), vim.log.levels.WARN)
  end
